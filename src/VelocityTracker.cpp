...
void VelocityTracker::OnDrawMenu() {
    ...
    if (ImGui::Button(ICON_MD_LOCAL_FIRE_DEPARTMENT " VelocityTracker")) {
        m_ShowMessage = !m_ShowMessage;
    }
}

void VelocityTracker::OnDrawUI(bool p_HasFocus) {
    if (m_ShowMessage) {
        // Show a window for our mod.
        if (ImGui::Begin("VelocityTracker", &m_ShowMessage)) {
            // Only show these when the window is expanded.
            ImGui::Text("Hello from VelocityTracker!");
        }
        ImGui::End();
    }
}
